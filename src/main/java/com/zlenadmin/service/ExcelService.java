...
//			Date daysAgo = cal.getTime(); 
	...
	public ByteArrayInputStream loadUserDetails(String deviceType, String userMobile, String zlenCode, String userName,
			String gender, Integer age, Integer age1, Integer friendNumber, Integer friendNumber1, Integer pageid,
			Integer total, @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm") Date fromdate,
			@DateTimeFormat(pattern = "yyyy-MM-dd HH:mm") Date todaydate,
		...
		List<UsersDetailDto> userDetails1 = userDetails.getUserDetails(userName, userMobile, zlenCode, deviceType,
				createdOn, gender, valueAge, valueAge1, valuefriendNumber, valuefriendNumber1, total, pageid, fromdate,
				todaydate);
	...
	public ByteArrayInputStream loadUserPerDayData(@DateTimeFormat(pattern = "yyyy-MM-dd HH:mm") Date todaydate,
		...
		List<UserPerDayCountDataDto> UserPerDayCountDataList = userDetails.getUserPerDayCountDataDto(todaydate,
				fromdate);
	...
	public ByteArrayInputStream loadUserStories(String zlenCode, String mimeType,
			@DateTimeFormat(pattern = "yyyy-MM-dd") Date uploadedDateTime, boolean zlenWorld, String userMobile,
			Date fromdate, Date todaydate) {
	...
	public ByteArrayInputStream loadPollVoteViewDownload(long id) {

		List<PollVoteDto> pvdList = poll.getPollVote(id);
			
			ByteArrayInputStream in = ExcelHelper1.pollVoteViewDownloadToExcel(pvdList);
			   return in;
		}
}
