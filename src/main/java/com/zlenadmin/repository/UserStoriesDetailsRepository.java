...
public interface UserStoriesDetailsRepository extends JpaRepository<UserStoriesDetails, Long> {
	...
	@Query(value="select count(usd.id) as count, DATE(usd.uploaded_date_time) as createDate, usd.video_type as mime_type from public.user_stories_details usd group by DATE(usd.uploaded_date_time), usd.video_type order by DATE(usd.uploaded_date_time) desc ", nativeQuery=true)
