...
public interface UserStoriesDetailsRepository extends JpaRepository<UserStoriesDetails, Long> {
	...
	@Query(value="select usd.mime_type, DATE(usd.uploaded_date_time), usd.uploaded_path, ud.user_name from user_stories_details usd inner join user_details ud on usd.user_id = ud.user_id", nativeQuery=true)
